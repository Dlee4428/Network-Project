...
int main(void)
{
	...
	do {
		iResult = recv(ClientSocket, recvbuf, sizeof(MySerialization), 0);
		if (iResult > 0) {
			printf("Bytes received: %d\n", iResult);

			MySerialization*msg = reinterpret_cast<MySerialization*>(recvbuf);
			
			// CALCULATE HP DAMAGE INFLICT INTO SERVER
			msg->plantHP = msg->plantHP - msg->zombieDAMAGE;
			msg->zombieHP = msg->zombieHP - msg->plantDAMAGE;
			...
			iSendResult = send(ClientSocket, recvbuf, sizeof(MySerialization), 0);
